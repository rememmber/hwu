...
class NEATGenotype(object):
    ...
    def get_weights(self):
	    return NeuralNetwork(self).get_connectivity_matrix();
...
class NEATPopulation(SimplePopulation):
    ...
    def _evolve(self, evaluator, solution=None):
        ...
        for specie in self.species:
            ...
            specie.avg_fitness = np.mean( map(float, [ind.stats['fitness'] for ind in specie.members]) )
            specie.max_fitness = np.max( map(float, [ind.stats['fitness'] for ind in specie.members]) )
    ...
    def _status_report(self):
        ...
        print "Best (%.2f): %s %s" % (float(self.champions[-1].stats['fitness']), self.champions[-1], self.champions[-1].stats)
