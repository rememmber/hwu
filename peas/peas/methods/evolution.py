...
class SimplePopulation(object):
    ...
    def _gather_stats(self, pop):
        ...
        for key in pop[0].stats:
            self.stats[key+'_avg'].append(np.mean(map(float, [ind.stats[key] for ind in pop])))
            self.stats[key+'_max'].append(np.max(map(float, [ind.stats[key] for ind in pop])))
            self.stats[key+'_min'].append(np.min(map(float, [ind.stats[key] for ind in pop])))
