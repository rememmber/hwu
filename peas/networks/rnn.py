...
class NeuralNetwork(object):
    ...
    def visualize(self, filename, inputs=3, outputs=1):
        ...
        for i in range(cm.shape[0]):
            ...
            for j in range(cm.shape[1]):
                w = cm[i,j]
                if abs(w) > 0.01:
                    G.add_edge(j, i, penwidth=1, color='black', label=w)
        print("inputs: "+str(inputs) + " outputs: "+str(outputs))
        for n in range(inputs):
            pos = (node_dist*n, 0)
            G.get_node(n).attr['label'] = 'input ' + '%s,%s!' % pos
        ...
        for i,n in enumerate(range(cm.shape[0]-outputs,cm.shape[0])):
            ...
            G.get_node(n).attr['label'] = 'output ' + '%s,%s!' % pos
            G.get_node(n).attr['pos'] = '%s,%s' % pos
            G.get_node(n).attr['shape'] = 'doublecircle'
    ...
    def get_connectivity_matrix(self):
	    ...
	    #self.visualize('visualize.png', inputs=10, outputs=2)
	    #print self.cm
	    #print(sys.getenv('PATH'))
	    #fifo = open('/Users/master/Desktop/test_processes/testing_stuff', 'w')
	    #print >> fifo, os.environ.get('PATH')
	    #fifo.close()
